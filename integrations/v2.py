...
class SearchView(CatalogView):
    def get(self, request, catalog):
        ...
        if len(query) > 200 or sort not in {"relevance", "newest", "quickest"}:
            raise ValidationError("q is limited to 200 characters; sort is relevance, newest or quickest.")
        try:
            limit = int(params.get("limit", 24))
            maximum = int(params["max_total_minutes"]) if "max_total_minutes" in params else None
            if not 1 <= limit <= 100 or (maximum is not None and not 1 <= maximum <= 100000):
                raise ValueError
        except ValueError:
            raise ValidationError("limit must be 1–100; max_total_minutes must be 1–100000.")
        ...
        if cursor:
            try:
                ...
                if payload["binding"] != binding:
                    raise ValueError
                ceiling, boundary, started = payload["ceiling"], payload["last"], payload["started"]
        ...
        recipes = search_recipes(recipes.filter(pk__lte=ceiling, finalized_at__lte=started), query)
        if maximum is not None:
            recipes = recipes.filter(total_time__lte=maximum)
        if sort == "quickest":
            recipes = recipes.annotate(
                time_unknown=Case(When(total_time__isnull=True, then=Value(1)), default=Value(0), output_field=IntegerField()),
                sort_time=Coalesce("total_time", Value(0)),
            )
            order = ["time_unknown", "sort_time", "-id"]
        else:
            order = (["-search_rank"] if sort == "relevance" else []) + ["-finalized_at", "-id"]
        if boundary:
            after, prefix = Q(pk__in=[]), Q()
            for field, value in zip(order, boundary, strict=True):
                name = field.lstrip("-")
                after |= prefix & Q(**{name + ("__lt" if field.startswith("-") else "__gt"): value})
                prefix &= Q(**{name: value})
        ...
        if len(rows) > limit:
            ...
            next_cursor = signing.dumps(
                {"binding": binding, "ceiling": ceiling, "started": started, "last": values}, salt="catalog-v2", compress=True
            )
        return Response({"results": [card(r) for r in rows[:limit]], "next_cursor": next_cursor})
...
class RecipeView(CatalogView):
    def get(self, request, public_id):
        ...
        if recipe.archived_at or recipe.visibility != "public":
            return Response({
                "document": None,
                "status": {"recipe_id": str(public_id), "availability": "unavailable", "feedback": None},
            }, status=410)
        try:
            document = recipe_document(recipe)
        except DocumentError:
            error = APIException("This recipe document is temporarily unavailable.", code="document_unavailable")
            error.status_code = 503
            raise error
        return Response({
            "document": document,
            "status": {"recipe_id": str(public_id), "availability": "available", "feedback": None},
        })


class V2ContractView(ContractView):
    def get(self, request):
        return Response(json.loads((settings.BASE_DIR / "docs/api/catalog-v2.openapi.json").read_text()))


class DocumentSchemaView(ContractView):
    def get(self, request):
        return Response(json.loads((settings.BASE_DIR / "docs/api/RecipeDocumentV1.schema.json").read_text()))
