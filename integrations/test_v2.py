...
@override_settings(CATALOG_AUDIENCE="http://testserver/api/integrations/v1/", CATALOG_V2_AUDIENCE="http://testserver/api/integrations/v2/")
class V2Tests(Setup, TestCase):
    def setUp(self):
        super().setUp()
        self.token = self.issue(resource=settings.CATALOG_V2_AUDIENCE).json()["access_token"]
        self.grant.minute_limit = 200
        self.grant.save()

    def read(self, path="catalogs/approved/recipes", **params):
        return self.client.get(BASE + path, params, HTTP_AUTHORIZATION="Bearer " + self.token)

    def add(self, title, ingredients=None, time=None, grant=True):
        starter = StarterRecipe.objects.create(
            slug=f"recipe-{StarterRecipe.objects.count()}", title=title,
            content={"title":title, "ingredients": ingredients or ["Salt"], "instructions":["Stir", "Serve"], "total_time":time},
    ...
    def test_search_shared_phrase_semantics_and_grants(self):
        ...
        self.assertEqual(public.json()["results"][0]["public_id"], str(ingredient_match.public_id))
        self.assertEqual(starters.json()["results"][0]["public_id"], str(ingredient_match.public_id))
    ...
    def test_lossless_document_stable_ids_personal_edits_and_audit(self):
        ...
        Recipe.objects.filter(pk=recipe.pk).update(notes="Private note", favorite=True, version=9)
    ...
    def test_keyset_pagination_ties_new_rows_and_parameter_binding(self):
        ...
        for sort in ["newest", "quickest", "relevance"]:
            ...
            self.assertEqual(self.read(q="Other", sort=sort, limit=2, cursor=cursor).status_code, 400)
            self.assertEqual(self.read(q="Same", sort=sort, limit=3, cursor=cursor).status_code, 400)
            self.assertEqual(self.read(q="Same", sort=sort, limit=2, cursor=cursor+"x").status_code, 400)
            self.assertEqual(self.read(q="Same", sort=sort, limit=2, cursor=cursor).json(), second)
    ...
    def test_time_filter_unknowns_and_no_unsupported_features(self):
        ...
        self.assertEqual([r["title"] for r in self.read(max_total_minutes=10).json()["results"]], ["Fast"])
        for params in [{"sort":"top_rated"}, {"tags":"dish:soup"}, {"min_votes":0}, {"after":1}, {"limit":0}, {"limit":101}, {"q":"x"*201}, {"max_total_minutes":0}, {"cursor":"bad"}]:
            self.assertEqual(self.read(**params).status_code, 400, params)
        self.assertEqual(self.read("catalogs/approved/recipes?q=a&q=b").status_code, 400)

    def test_availability_grant_revocation_and_private_boundaries(self):
        recipe = self.starter.recipe
        path = f"recipes/{recipe.public_id}"
        for changes in [{"visibility":"private"}, {"visibility":"public", "archived_at":timezone.now()}]:
        ...
        private = Recipe.objects.create(owner=get_user_model().objects.create_user("owner"), title="private")
    ...
    def test_same_quota_kill_switch_audience_and_read_only(self):
        ...
        self.grant.save()
        for field,value in [("resource",["https://wrong.example/"]), ("expires",timezone.now()-timedelta(seconds=1)), ("scope","recipes:write")]:
            token = AccessToken.objects.get(application=self.app, resource=[settings.CATALOG_V2_AUDIENCE])
            old = getattr(token,field)
            setattr(token,field,value);token.save()
            self.assertIn(self.read().status_code,[401,403])
            setattr(token,field,old);token.save()
        self.assertEqual(self.client.post(BASE+"catalogs/approved/recipes", {}, format="json", HTTP_AUTHORIZATION="Bearer "+self.token).status_code,405)
        self.grant.enabled=False;self.grant.save()
    ...
    def test_overlapping_catalogs_and_cursor_cannot_cross_apps_or_catalogs(self):
        ...
        self.assertEqual(self.read("catalogs/second/recipes",limit=1,cursor=first["next_cursor"]).status_code,400)
        self.assertEqual(self.read(f"recipes/{self.starter.recipe.public_id}").status_code,200)
        self.grant.catalogs.clear()
        self.assertEqual(self.read(limit=1,cursor=first["next_cursor"]).status_code,404)
    ...
    def test_contract_and_full_seed_projection(self):
        ...
        for recipe in recipes:
            ...
            self.assertEqual([i["original_text"] for i in document["ingredients"]],recipe.ingredients)
            self.assertEqual([s["original_text"] for s in document["steps"]],recipe.instructions)
        self.assertEqual(self.client.get(BASE+"openapi.json").json()["info"]["version"],"2.0.0")
        self.assertEqual(self.client.get(BASE+"recipe-document.schema.json").status_code,200)
